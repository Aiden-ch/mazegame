...
public class MazeCombat implements Screen {
	...
	public MazeCombat(MazeGame gaeme) {
		...
		Melee swordMelee = new Melee(swordImage, sword, 10d, 1f, (float)Math.PI/2);
		...
		blobEnemy = new Enemy(blob, blobImage, 5f, 5f, 0.5d);
	...
	public void render(float delta) {
		...
		//enemy spawn
		if(time % 40 <= 0.5) { //change num after mod to change spawn speed
		...
		for(int i=0; i<AttackHandler.getCards().size(); i++) {
			...
			if(temp.getType() == 'p') { //projectiles
				...
				if (temp.getMel().swing()) { //might cause problems if fight lasts too long 
					...
					for(int j=0; j<EnemyHandler.getEnemies().size(); j++) {
						if(temp.getMel().getBox().overlaps(EnemyHandler.getEnemies().get(j).getBox())) {
