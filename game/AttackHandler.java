...
//			projImage.setOrigin(item.getProj().txte.getWidth()/2.0f, item.getProj().txte.getHeight()/2.0f);
	...
	public static float melee(Player player, Item item, float buffer) {
		...
		if(Gdx.input.isButtonPressed(Input.Buttons.LEFT) && buffer <= 0.0f) {
			...
			meleeImage.setRotation((float)((float)(180 / Math.PI * item.getMel().getRadius()) + 
							Math.atan2(Gdx.graphics.getHeight()-Gdx.input.getY()-player.getYPos(), Gdx.input.getX()-player.getImage().getWidth()/2-player.getXPos()) * 180 / (Math.PI)));
			...
			Melee mel = new Melee(item.getMel().getDamage(), item.getMel().getLength(), 
					item.getMel().getRadius(), item.getMel().getSpeed(), meleeImage, item.getMel().getTexture());
